...
To keep the road ownership consistent between the regional and city data, the adjustments are made firstly among the common IDs and then based on the length and geometry with a precision setting on each to make sure the exact one-on-one match. The remain mismatches are caused by extra segments in either dataset or different road segments with the same ID on the same road. These mismatches are usually in small segments, which are considered minor issues and ignored. More work might need to be done to make sure the extra road segments in either are matched. 
